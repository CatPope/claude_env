...
1. **훅 출력 형식**: `sessionTitle` 은 최상위가 아니라 `{"hookSpecificOutput":{"hookEventName":"SessionStart","sessionTitle":"..."}}` 안에 둔다(`SessionStart` startup/resume 는 v2.1.152+, `UserPromptSubmit` 은 v2.1.94+; Stop/Notification/SessionEnd 에는 문서화되지 않음). 반면 `terminalSequence` 는 **최상위 키**다(`{"terminalSequence":"\u001b]0;...\u0007"}`, OSC 0/1/2/9/99/777 + BEL). 스크립트는 두 형식을 각각 그대로 쓴다.
